...
### 공통 (root layout / 에러 처리)

- 전체 앱은 `<html translate="no">` + `google: notranslate` meta로 브라우저 자동번역을 차단한다 — Chrome 번역이 React가 관리하는 DOM 텍스트 노드를 font 태그로 감싸, 리스트 재구성이 큰 커밋(리뷰 토글 등)에서 removeChild 계열 크래시를 일으키는 것을 막는 조치다. 한국어 전용 연구 도구라 번역 필요가 없다 `[추가 2026-07-25 → 15.336]`
- `src/app/error.tsx`(세그먼트)와 `global-error.tsx`(루트 layout 대체)가 uncaught 렌더 예외 시 전면 백지 대신 다시 시도/새로고침 안내 fallback을 보여준다. 리뷰 draft는 서버 저장 범위까지 복원된다는 안내를 포함한다 `[추가 2026-07-25 → 15.336]`
...
- 미션 클릭 → `/main/[missionId]`로 이동 `[stale 2026-06-12 → 15.65: 순차 잠금 도입. 잠긴 미션은 이동하지 않고 lockReason toast만 표시(onLockedClick). 첫 미완료 1개만 해제, 완료 미션은 review만]` `[변경 2026-07-24 → 15.333: 잠금 해제(클리어) 조건을 완료 + 해당 미션 메모리 리뷰 제출로 강화. 첫 미클리어 미션(미완료 또는 완료-리뷰 미제출)이 현재이고, 완료-리뷰 미제출 미션이 막고 있으면 lockReason이 메모리 리뷰 제출을 안내]`
...
- 사용자 카드는 1열 전체 폭으로 배치한다. 카드의 미션 영역은 온보딩을 첫 행에 두고 `missionOrder` 순서를 기준으로 참여/세션 미션을 보완한 단일 진행 목록이다. Lobby와 같은 session snapshot 판정으로 `대기`/`준비중`/`진행중`/`시간 초과`/`완료`를 표시하고, 온보딩 미션도 Lobby처럼 실제 세션 진행을 반영하되 완료 판정만 onboarding profile flag로 한다. Lobby의 순차 잠금 규칙(온보딩→`missionOrder` 순서, 첫 미완료가 `현재`, 그 이후는 `잠김`)도 같은 판정으로 계산해 `현재`/`잠김` 배지와 흐릿 처리로 표시한다(관리/조회용이라 잠금은 표시만 하고 링크는 막지 않음). `[변경 2026-07-24 → 15.333: 현재 판정도 로비와 동일하게 미클리어(미완료 또는 완료-리뷰 미제출) 기준으로 강화]` 각 행의 미션 제목 링크는 `/main/{id}?viewAs={uid}`로 해당 세션을 읽기 전용 view-as로 연다. admin viewAs 세션의 헤더 뒤로가기 버튼은 `/admin`으로 돌아가며, read-only banner 안의 별도 `어드민으로 돌아가기` 링크는 두지 않는다(별도 리뷰 링크는 제거 — admin viewAs는 이미 읽기 전용+리뷰 탭 노출이라 `review=1`은 초기 탭만 바꿔 중복이었다) `[현행 2026-07-04 → 15.122/15.123/15.124/15.125/15.127/15.181]`
- `[stale 2026-07-17 → 15.292: 참여자 모달과 개별 미션 기록 삭제 기능 제거(구 15.94/15.197 동작)]`
- 유저 목록과 리뷰 답변 목록은 참가자를 먼저 보여주고, 관리자 계정(`ADMIN_EMAILS`)은 하단의 `관리자` 구분선 아래 별도 섹션으로 분리한다. 리뷰 답변의 참가자 드롭다운에서도 관리자는 뒤로 정렬되고 `· 관리자` 접미가 붙는다 `[현행 2026-07-21 → 15.320]` 리뷰 답변의 답변별 보기는 관리자를 아예 제외한다 — 드롭다운에서도 숨기고, 관리자가 선택된 채 답변별로 전환하면 전체 참가자로 되돌린다. 전체 참가자 상태의 Likert 문항(1·2·3·10)은 참가자별 그래프를 나열하지 않고 세션 순번별 전체 평균 그래프 하나만 표시한다(개별 그래프·이유 텍스트는 특정 참가자 선택 시에만) `[현행 2026-07-28 → 15.341]` 리뷰 답변 탭 맨 위에는 참가자별 진행률 스트립(P번호·이름·진행 바·완료 N/전체, 관리자 제외)을 표시하며, 완료 판정은 유저 카드와 같은 규칙이다. `HIDDEN_ADMIN_VIEW_EMAILS`(src/lib/admin.ts)의 폐기 계정은 관리자 유저 목록·참가자 번호 부여·리뷰 뷰에서 코드 레벨로 완전히 제외된다 — Auth 재로그인으로 빈 프로필이 재생성돼도 다시 나타나지 않는다 `[현행 2026-07-28 → 15.342]` 참가자에게는 Firebase Auth 가입 시각(들어온 순서)으로 P1, P2, ... 번호를 부여해 이름 앞에 표시하고 P1이 상단에 오도록 정렬한다. 이 번호는 저장하지 않고 조회 시점에 파생하는 관리자 페이지 표시 전용 값이며 참가자 화면에는 노출되지 않는다 `[현행 2026-07-21 → 15.321]`
...
- 우측 패널 (고정): AI 에이전트 채팅
- 완료 세션 리뷰 우측 패널은 `세션 이전`/`채팅` 탭만 제공한다. 사용자용 `리뷰 보기`/`메모리 리뷰하기` CTA는 먼저 backdrop 없는 Part 1 패널을 열어 오늘 세션 이해도, memory 도움도, 앞으로 기억할 내용을 받는다. Part 1의 1~7점 척도는 숫자 버튼과 함께 `1 전혀 아니다`/`4 보통`/`7 매우 그렇다` 지표를 표시한다. 문항 구성은 `[stale 2026-07-17 → 15.303: 1 작업 방식, 2 디자인 취향·선호, 3 메모리 덕분에 협업 수월(Likert), 4문항 배치는 Likert 1~3을 md 이상 가로 3열 한 줄로, 4 자유응답을 그 아래 전체 폭 행으로. 헤더는 Review Part 1 · 오늘 세션 돌아보기 한 줄, 부제 제거]` `[stale 2026-07-24 → 15.330: Part 1을 두 스텝으로 분할 — 스텝 1은 Likert 1~3만(md 이상 가로 3열), 스텝 2는 자유응답 4만 표시. 스텝 1 다음은 Likert 3개 완료 시 스텝 2로 로컬 전환, 스텝 2의 이전/다음으로 왕복하며 다음이 Part 2 진입. 헤더 우측에 스텝/2 표시]` 이 패널은 왼쪽 작업/리뷰 영역과 오른쪽 채팅을 덮지 않도록 header 아래 전체폭 strip으로 표시하고, 열 때는 채팅 탭으로 전환한다. Part 1에는 닫기 X를 두지 않고 `다음`으로만 Part 2에 진입한다. `다음`을 누르면 Part 1 답변을 `users/{uid}/memoryReviewFeedback/{missionId}.answers`에 draft 저장한 뒤 cluster list → detail panel → memory graph → Part 2 review panel 순서의 full-screen memory overlay를 연다. 리뷰 오버레이의 열림 의도는 `mreview=intro|memory` URL 파라미터로도 기록된다 — 가시성은 state가 즉시 제어하고, `?review=1` 네비게이션 도중 페이지 리마운트로 overlay state만 리셋되어 세션 리뷰 화면으로 떨어지던 간헐 이슈를 URL 기반 sync effect가 복원한다(진입 시 set, Part 1→2 전환 시 memory로 교체, 오버레이 닫기 시 제거; admin viewAs는 종전대로 state-only) `[추가 2026-07-24 → 15.331]` Part 2 review panel header는 Part 1의 자유응답(4번) 실제 텍스트를 따옴표로 넣어 `‘답변’와 관련해, 에이전트는 지금 이렇게 기억하고 있어요.`라고 표시하고, 본문은 바로 5번 문항부터 시작한다 `[변경 2026-07-17 → 15.303]`. Part 2 질문 원문과 순서는 `src/components/memory/memory-review-panel.tsx`의 `REVIEW_QUESTIONS`가 source of truth다. memory 상태 확인 문항은 8번(rating 문항 직전)에 위치하고 `[변경 2026-07-17 → 15.287]`, detail card에서 활성 memory를 사유 입력 후 비활성화하거나 inactive memory를 재활성화할 수 있다. `[변경 2026-07-20 → 15.310: 토글은 제출 전까지 서버에 반영되지 않는 staging이다 — 화면에만 반영하고 카드에 제출 시 반영 배지를 붙이며, staged 상태와 토글 이벤트 이력은 memoryReviewFeedback draft의 memoryActivations 필드로 저장돼 재진입 시 복원된다. 최초 제출 시 최종 상태를 메모리 문서에 일괄 적용(재활성화 weight 0.5)하고 after snapshot 재생성을 best-effort 호출한다. 재활성화 토글마다 staged 상태를 가정한 preview 클러스터링(session-clusters preview 모드, 저장 없음)을 호출해 화면 after snapshot을 즉시 갈아끼우고 새 소속 클러스터로 focus를 옮긴다 — preview 도착 전/실패 시에는 비활성 그룹 자리에 활성 스타일로 남는다]` `[변경 2026-07-25 → 15.336: draft의 memoryActivations 저장은 서버에서 memoryId별 merge다 — 리마운트 직후처럼 일부 토글만 담긴 payload가 와도 기존 staged 토글을 지우지 못하고, undo(항목 제거)는 더 최신 토글 이력이 함께 올 때만 반영된다. 클라이언트는 draft 복원(GET) 완료 전까지 memoryActivations 전송을 생략하고 토글 UI도 숨기며, 토글 임시 저장 실패는 toast로 알린다]` historical before snapshot은 유지한다. Overlay의 `세션 이전`/`세션 이후` 토글은 같은 최신 cluster cache를 node filter로 재사용하지 않고, `users/{uid}/memoryClusterSnapshots/{missionId}_{before|after}`에 저장된 phase별 cluster snapshot을 우선 사용한다. Snapshot이 없는 과거 세션은 기존 latest cache fallback으로 표시한다. `세션 이전` 탭의 `원래 입력한 내용`은 현재 미션의 `profile_memories/{missionId}` 원문만 표시하며, 누적 before-session graph memory의 과거 raw input을 현재 미션 입력처럼 대체 표시하지 않는다. Part 2 리뷰 입력창에서 `@`를 입력하면 별도 dropdown 없이 메모리뷰 자체가 선택 모드가 되며, cluster list/detail panel/graph에서 cluster나 memory를 클릭해 답변 본문에 inline mention을 삽입한다. 삽입된 mention은 굵게 표시되고 클릭하면 해당 cluster나 memory로 focus된다. 답변은 Part 1 3개와 Part 2 7개 문항별 plain text와 structured mentions로 draft 저장된다. 모든 Part 2 질문을 입력해야 제출할 수 있고, 제출 확인 팝업에서 확정하면 `submittedAt` 저장 후 로비로 이동한다. Admin viewAs는 관측용이므로 Part 1 없이 기존 full-screen overlay를 바로 연다. 로비 완료 미션 카드, admin 유저 카드의 완료 미션 row, admin 참여자 row는 `submittedAt` 여부에 따라 `리뷰 필요` 또는 `리뷰 완료`를 표시한다 `[현행 2026-07-16 → 15.131/15.134/15.155/15.156/15.165/15.166/15.167/15.183/15.186/15.197/15.242/15.260/15.270/15.277/15.278]`
...
- 현재 미션 모델: 온보딩 제외 9개 단독 미션, 각 미션 옵션 1개. 유저별 랜덤 순서로 순차 진행(잠금). `[현행 2026-06-12 → 15.64/15.65]` 다음 미션 잠금 해제는 이전 미션의 세션 완료만이 아니라 메모리 리뷰 제출(`memoryReviewFeedback.submittedAt`)까지 요구한다 — 리뷰를 미룬 채 다음 미션을 시작한 뒤 이전 리뷰에서 메모리를 비활성화하면 진행 중 세션의 retrieval 조건이 소급 변형되기 때문. 온보딩 리뷰도 동일 적용 `[현행 2026-07-24 → 15.333]`
...
- 레퍼런스 검색 턴에 사용자가 스타일 이미지를 첨부하면 클라이언트가 `/api/references`에 이미지 data URL을 함께 보내고, 서버가 vision 분석으로 검색용 스타일 단서를 만든 뒤 검색 context와 query builder 입력에 합친다. `[변경 2026-07-24 → 15.335: 최대 3장 첨부 가능 — styleImages 배열로 보내고 vision 분석이 여러 장의 공통 스타일 방향을 한 문구로 합친다. 단일 styleImage는 구버전 폴백]` 생성된 단서는 해당 assistant chat bubble의 `이미지 스타일 검색 기준` 섹션에도 표시한다. 분석 실패 또는 이미지 형식/크기 제한 초과 시 기존 텍스트 기반 검색으로 폴백한다 `[현행 2026-07-04 → 15.185]`
...
- 선택을 바꾸는 중간 클릭은 memory draft를 만들지 않는다. 세션 종료 직전에 현재 `finalArtboardId` 하나만 final-design memory로 기록하며, 종료 API도 과거 방식으로 누적된 선택 draft 중 현재 최종안 하나만 승격한다 `[현행 2026-06-23 → 15.112]` 종료 재시도 사이에 최종안이 바뀌면 이전 시도가 이미 승격한 final-design memory는 `archiveReason: final_design_superseded`로 자동 비활성화(archive)해 세션당 최종안 memory 1개 계약을 유지한다 `[현행 2026-07-25 → 15.337]`
...
- **확정 시점**: 사용자가 `미션 종료` 버튼(구 `세션 종료`)을 누르면 `/api/memory/complete-session`에서 draft를 통합해 장기 메모리로 저장. 온보딩 제외 미션에서 미션 종료 버튼은 제한 시간 2분 전(30분 미션이면 28분)까지 비활성화되고 `[현행 2026-07-22 → 15.328]`, 제한 시간(`missionDurationMinutes`, 기본 30분) 전에 누르면 먼저 조기 종료 확인 문구가 뜨며, 확인 후 목업이 있는데 최종 디자인 미선택이면 기존 확인이 이어진다 `[현행 2026-07-21 → 15.325]` complete-session은 멱등 재시도를 지원한다 — 이미 promoted인 draft는 스킵하고, 임베딩은 draft별 병렬 호출이 아니라 배치 1회이며(실패 시 빈 embedding으로 승격 — retrieval/clustering의 `ensureFreshMemoryEmbeddings` 자가 치유에 위임), draft별 쓰기는 실패 격리 + 동시성 제한으로 수행하고, 하나라도 실패하면 세션을 completed로 마킹하지 않고 명시적 실패를 반환해 재시도가 실패분만 이어서 처리한다. final-design draft의 인코딩 LLM 실패는 `/api/memory/drafts`가 사실 기반 fallback(episode = 원문 입력, `encodeError` 마커)으로 저장해 미션 종료를 하드 블록하지 않는다 `[현행 2026-07-25 → 15.337]`
...
- **Forgetting / inactive memory**: active memory의 기준은 `archivedAt`이 없고 `weight > 0`인 문서다. `archivedAt`이 있거나 `weight <= 0`인 memory는 Firestore에는 남지만 retrieval과 clustering 입력에서 제외된다. `/agent`와 admin의 사용자 전용 memory page는 관측을 위해 명시적으로 inactive/archived 문서도 함께 읽되 실제 similarity cluster membership과 edge에서는 제외하고 `session-inactive` pseudo-group으로만 보여준다. `weight <= 0`은 별도 archive write 없이 inactive로 취급한다. Idle decay는 상시 작동하는 목표 곡선 컨트롤러다 `[변경 2026-07-21 → 15.322]`: retrieve되지 않은 memory는 매 턴 깎이며, 속도는 활성 수와 목표 곡선의 관계로 정해진다 — 활성이 목표 이상이면 `0.009`~`0.018`(초과분 50개에서 포화) 램프, 목표 미만이면 `0.009 × √(활성/목표)`로 감속하되 멈추지 않는다 `[변경 2026-07-21 → 15.323/15.324]`. 좋아요/싫어요로 weight가 출렁여도 활성 풀이 목표 곡선으로 수렴한다. 목표 곡선은 `40 + 0.4 × 전체 입력`의 선형 구간(고정점: 100개→80, 200개→120)과 200 이후 제곱근 완만 구간(300개→~142)이다 `[변경 2026-07-21 → 15.324: 초기 목표를 낮춰 바닥 코호트가 전체 70개 부근부터 소멸 구간에 들어가게 조정]`. 전체 입력 수는 비활성 포함 문서 수(retrieval 로그의 totalMemoryCount/activeMemoryCap 필드)다. 사용자는 세션 memory review의 detail card에서 직접 memory를 비활성화할 수 있고, 이때 자유 입력 이유를 필수로 받아 `inactiveReason: user_disabled`와 `inactiveReasonDetail`을 저장하며 weight를 0으로 만든다. 재활성화는 archive/manual/weight-zero 원인과 관계없이 `archivedAt`/비활성 marker를 해제하고 weight를 0.5로 복구한다. 이 적용은 토글 즉시가 아니라 리뷰 최초 제출 시 일괄로 일어난다 — 그 전까지는 클라이언트 staging + `memoryReviewFeedback/{missionId}.memoryActivations` draft에만 존재한다 `[변경 2026-07-20 → 15.310]`. draft 저장은 서버에서 memoryId별 merge라 복원 전 클라이언트의 sparse payload가 이전 staged 토글을 지우지 못한다 `[추가 2026-07-25 → 15.336]`. 제출 시 undo 포함 토글 이벤트 전체가 `users/{uid}/memoryActivationLogs/{toggledAt}-{memoryId}`에 append-only로 기록된다(memoryId, active, reason, 리뷰 세션 missionId, 클라이언트 toggledAt, applied/appliedAt, 적용 직전 active/weight) — 메모리 문서는 마지막 상태만 남기고 재활성화가 inactiveReason을 지우므로 토글 시점/세션 이력은 이 로그가 유일한 근거다 `[현행 2026-07-20 → 15.309/15.310]`. 세션 리뷰와 `/agent`의 inactive memory는 별도 pseudo-group이라 cluster 수에 포함하지 않는다. 리뷰 overlay의 비활성 pseudo-group은 세션 이전/이후 페이즈 모두에서 누적 기준으로 표시된다 — before(N)=after(N-1) 등식(15.318)에 맞춘 것이며, 이번 세션발 비활성 +N 배지만 after 전용이다 `[현행 2026-07-28 → 15.343]`. 리뷰 overlay의 노드 활성/비활성 판정은 현재 문서 상태(weight/archivedAt)가 아니라 해당 phase snapshot의 itemIds 멤버십 기준이다 — 스냅샷 입력은 생성 시점의 활성 메모리만 담으므로 멤버십이 그 시점 상태의 동결 기록이고, 현재 상태로 판정하면 과거 세션 리뷰에서 이후 세션의 decay 사망이 소급 표시되어 클러스터가 무너진다. 멤버는 현재 죽었어도 클러스터에 활성으로 남고, 비멤버(누적 스코프 내)가 비활성 그룹에 표시된다. 스냅샷이 없는 legacy 세션만 현재 상태로 폴백하며, detail card의 weight %와 비활성 사유 텍스트는 여전히 현재 문서 값이다(시점별 weight는 미저장) `[현행 2026-07-29 → 15.345]`. +N의 세션발 판정은 weight 0 전체가 아니라(상시 decay 이후 누적 사망까지 세는 오류) 세션 처리 중 아카이브, 세션 이전 스냅샷 멤버였다가 비활성, 이번 세션 생성 후 비활성만 센다 `[현행 2026-07-28 → 15.344]` `[변경 2026-07-29 → 15.345: 비활성 판정이 after snapshot 멤버십 기준이 되어 (before ∪ promoted) − after 집합 연산과 동일 — 과거 세션을 나중에 봐도 이후 사망을 세지 않는다]`. cluster list 하단의 보조 행은 inactive memory가 0개여도 유지하며 count 0을 표시한다. 이 행을 누르면 inactive pseudo-group을 선택해 detail panel을 전환하고, memory가 있으면 graph node도 보이게 한다. 비활성 pseudo-group의 detail panel에는 설명 문구 아래 비활성 사유 chip 필터(전체 / 유사 메모리 정리 / weight 0 도달 / 사용자 비활성화, 건수 표시, 0건 chip은 비활성)가 있어 카드 목록을 사유별로 좁힌다. 이 필터는 연구 관측 화면인 `/agent`와 admin 사용자 메모리 페이지에서만 노출된다(`MemoryClusterSidePanel`의 opt-in prop `showInactiveReasonFilter`, `MemoryClusterPage` 렌더에서만 켬) — 참가자용 세션 리뷰 overlay에는 표시하지 않는다. 사유 판정은 카드의 비활성 이유 라벨과 같은 분류 키를 공유하며(user_disabled > archivedAt/archived action > weight 0), 그래프 노드 클릭으로 필터에 걸러진 메모리를 새로 선택하면 필터가 자동 해제되어 카드가 보인다. 그룹을 벗어나면 필터는 초기화된다 `[추가 2026-07-27 → 15.338]` 행 오른쪽 eye icon은 선택과 별개로 node 표시만 토글하고 0개일 때는 비활성화하며, list를 접으면 group 선택 icon과 eye toggle을 하단에 유지한다. 상태 변경 command는 세션 리뷰의 일반 사용자에게만 제공하고 `/agent`와 admin view에는 제공하지 않는다. Review turn에 이미 저장된 retrieved memory는 리뷰 패널에서 회색 inactive 상태와 이유를 표시한다. Archived/inactive node와 detail card는 phase weight와 무관하게 더 옅은 회색 톤으로 렌더한다. Admin forgetting 후보/수동 archive route와 탭은 제거되었고, admin은 current graph/table/retrieval log 중심으로 관측한다 `[현행 2026-07-16 → 15.202/15.211/15.235/15.241/15.269/15.277/15.279/15.280/15.281/15.282/15.283/15.286]`

#### 메모리 클러스터링

- 경로: 일반 사용자 본인 memory는 `GET/POST /api/memory/clusters`, admin의 타인 memory 진단은 `GET/POST /api/admin/users/[uid]/memory/clusters`
- 입력: clustering vector는 retrieval과 같은 active `memories_0_1_2.embedding` 저장값을 사용한다. `archivedAt`이 있거나 `weight <= 0`인 memory는 현재 graph/clustering 입력에서 제외한다. 저장값이 없거나 `embeddingSource`가 현재 sourceType 계약과 맞지 않으면 keyword/episodic/semantic/link(before-session은 source.sourceText 포함) 텍스트로 재생성해 문서에 write-back한다. 원문 interaction input/output은 clustering embedding에 포함하지 않고, 입력 variant 비교는 제공하지 않는다 `[현행 2026-07-13 → 15.178/15.201/15.202/15.211]` full 클러스터링(self/admin `clusters` API)과 세션 snapshot은 같은 미션 스코프를 공유한다 — 온보딩이거나 사용자 `missionOrder`에 있는 미션의 활성 메모리만 입력이 되고, `source.missionId`가 없거나 missionOrder 밖인 메모리는 양쪽 모두 제외된다. 입력 개수 상한(최신 300개)은 제거되어 스코프 안 활성 메모리 전체가 입력된다 `[현행 2026-07-21 → 15.315]`
...
- `/agent` cluster UI는 좌측에서 cluster list → detail panel → graph 순서로 배치한다. cluster list는 main 세션 리뷰와 동일한 `MemoryClusterList` review presentation을 사용한다 — 색상 count rail이 달린 rounded card, cluster label만 표시, 좌측 접기 rail 제공. cluster summary와 선택됨 badge는 숨긴다. Cluster color는 배열 index가 아니라 저장된 `colorIndex`를 우선 사용하며, cluster 재생성 시 이전 cluster와 item overlap이 가장 큰 successor 하나가 기존 색을 계승한다. 승계의 직전 세대는 snapshot 체인을 우선한다 — after-only 재생성(리뷰 제출)은 자기 after→before 순서(스테이징 preview와 동일), 세션 종료 생성은 직전 미션의 after snapshot이며, snapshot이 없을 때만 최신 full 캐시로 폴백한다 `[변경 2026-07-21 → 15.316]` Graph layout은 현재 edge weight 분포의 p05~p95 범위로 spring strength를 정규화하고, hard boundary clamp 대신 soft boundary force를 사용해 node가 viewport 가장자리에 일렬로 붙는 현상을 줄인다 `[현행 2026-07-13 → 15.169/15.238/15.239]`
- `/agent`(self·admin 공용)의 세션 칩은 해당 미션의 동결 after snapshot(`memoryClusterSnapshots/{missionId}_after`)의 cluster/edge를 그대로 렌더한다 — 세션 리뷰 overlay의 `세션 이후`와 같은 화면. 노드 가시성도 리뷰와 동일하게 snapshot 멤버이거나 현재 비활성인 메모리(비활성 pseudo-group)만 남긴다. 누적 범위는 유저별 `missionOrder` 기준이고, 선택 미션에서 새로 생긴 메모리만 다이아몬드/+N으로 표시한다. 기본 탭 `전체`는 마지막 완료 세션(missionOrder상 가장 뒤 after snapshot, generatedAt은 동순위 tiebreak)의 화면과 완전 동일하되 새 메모리 하이라이트만 없다 — 그 이후 생긴 다음 세션 before-session 메모리는 숨긴다. snapshot이 없는 미션과 세션 외 버킷만 기존 최신 `memoryClusters` 캐시 node 필터로 폴백한다. 세션 리뷰 overlay는 기존대로 phase별 before/after snapshot을 분리 표시한다 `[현행 2026-07-21 → 15.178/15.197/15.316]` before 스냅샷의 입력은 직전 세션 종료 시점과 동일하다 — 이번 미션에서 생긴 메모리는 사전정보(before_session) 파생을 포함해 전부 제외하므로, 정상 흐름에서 before(N)은 after(N-1)과 item set·클러스터·라벨까지 동일하다(동일 item set은 라벨/색을 그대로 재사용). 사전정보 파생 메모리는 after 스냅샷부터 나타난다. 2026-07-21 이전에 저장된 before 스냅샷은 사전정보를 포함한 옛 규칙 그대로 동결 유지한다 `[현행 2026-07-21 → 15.318]`
- 캐시 키는 memory version + item signature + clustering method version으로 관리하고, method version에는 고정 입력 이름 `keyword-episodic-semantic-link`가 포함된다. mean-centered adaptive-threshold graph 전환 후 method version은 `similarity-graph-v6-centered-adaptive-threshold`이며, latest fallback 조회도 같은 current method version만 읽어 과거 compact-context/full-context/v3/v4/v5 cache와 섞지 않는다 `[현행 2026-07-13 → 15.178/15.201/15.234/15.237]` 최신 `memoryClusters` 캐시의 용도는 retrieval cluster evidence와 snapshot 폴백이며, 메모리 뷰 표시는 after snapshot이 우선이다 `[현행 2026-07-21 → 15.316]`
...
### 15.303 Restructure Part 1 into four questions and shift Part 2 numbering `[implemented 2026-07-17]`

- Part 1 헤더를 Review Part 1 · 오늘 세션 돌아보기 한 줄로 합치고 부제(채팅을 보면서...) 제거.
- 기존 1번(취향+작업 방식 통합 Likert)을 둘로 분리: 1 작업 방식 이해(session_understanding 키 유지), 2 디자인 취향·선호 이해(신규 키 design_preference_understanding). 기존 2번은 3번이 되고 문구를 메모리 덕분에 협업이 더 수월했다로 변경(memory_helpfulness 키 유지). 자유응답은 4번(future_memory_freeform 키 유지).
- Part 2 startNumber 4→5로 문항 번호가 하나씩 밀림(5~11). Part 2 intro의 3번에서 적은 답변 참조 문구도 4번으로 갱신. memory-review-panel의 PART1_QUESTIONS(admin Part 1 요약 표시)도 4문항으로 동기화.
- 기존 저장 데이터 호환: 키를 유지한 문항은 draft가 그대로 프리필되고, 신규 키는 과거 세션에서 빈 값으로 표시된다.

### 15.304 Cut product reference hydration latency `[implemented 2026-07-17]`

- 증상: 레퍼런스 검색(특히 product 모드)이 40초대. 로그의 screenshot fallback timeout이 단서.
- 원인: 15.272의 product 모드 screenshot-first에서 (1) Microlink 콜드 렌더가 무거운 페이지 실측 ~20초인데 timeout 18초라 첫 조회는 항상 타임아웃으로 18초를 통째로 낭비, (2) 스크린샷 실패 후에야 직접 이미지 추출을 직렬로 시작(후보당 최악 18+7+5초), (3) 반환은 3개인데 6개 후보를 hydrate(이미지 실패 카드도 그대로 반환되므로 여분 hydrate는 순수 낭비, concurrency 3 기준 wave 2회).
- 수정: timeout 12초로 단축(abort해도 Microlink가 렌더를 마저 끝내 캐시하므로 같은 URL 재검색은 즉시 히트 — 실측 65ms), screenshot-first에서 직접 이미지 검증을 병렬 시작해 실패 시 즉시 폴백, hydrate 대상을 targetCount(3)로 축소해 wave 1회로.
- 효과: product 검색 hydration 최악 ~30초+×2wave에서 ~12초 1wave로. 오늘 함께 관찰된 retrieve 3.6초/chat 6.2초는 임베딩·LLM 지연이 대부분으로 금일 변경(semaphore, vision)의 영향은 수백 ms 이하.

### 15.305 Carry the most recent attached image into follow-up turns `[implemented 2026-07-17]`
...
### 15.309 Append-only memory activation toggle log `[implemented 2026-07-20]`

- 문제: 사용자 활성화/비활성화 토글이 메모리 문서의 마지막 상태 필드로만 남았다. 재활성화가 inactiveReason/inactiveReasonDetail을 null로 지우고, 재비활성화가 이전 사유를 덮어써서 언제/어떤 리뷰 세션에서 토글했는지 이력을 복원할 수 없었다.
- 구현: /api/memory/active-state가 메모리 patch 직후 users/{uid}/memoryActivationLogs/{now}-{memoryId}에 이벤트 문서를 append한다. 필드는 memoryId, active, reason(비활성화 사유, 활성화는 null), missionId(리뷰 세션, 클라이언트가 body로 전달), previouslyActive, previousWeight, createdAt. 로그 write 실패는 라우트 오류로 표면화된다(메모리 patch는 이미 적용된 상태).
- 데이터 수명주기 동기화: memoryActivationLogs를 admin 사용자 백업/삭제(USER_MEMORY_COLLECTIONS), mission-scoped 삭제(adminMemoryCleanup, missionId 필드 매칭), scripts/backup_users.py, scripts/wipe_users.py, scripts/export_sessions.py memory export에 모두 포함했다.
- 미구현: admin UI에는 아직 노출하지 않는다. 분석은 daily export의 memories.json 또는 Firestore 콘솔로 한다.
- `[stale 2026-07-20 → 15.310: 토글 즉시 적용/로그 방식에서 리뷰 제출 시 일괄 적용으로 변경. /api/memory/active-state 라우트는 제거됐고 적용과 로그는 review-feedback 라우트가 담당한다]`

### 15.310 Stage memory activation toggles until review submission `[implemented 2026-07-20]`

- 문제: 15.309까지는 토글이 즉시 메모리 문서에 적용돼, 리뷰를 제출하지 않고 이탈해도 장기기억이 바뀐 상태로 남았다. 리뷰 완료 여부와 메모리 변경이 세션 단위 분석에서 어긋난다.
- 클라이언트: 토글은 서버 호출 없이 staging만 한다. summary/archive-status 화면 상태를 로컬로 덮어쓰고(undo 시 baseline 복원), 카드에 amber 제출 시 반영 배지를 표시한다. 클러스터 스냅샷에 없던 메모리를 재활성화하면 제출 전까지 비활성 그룹 자리에 활성 스타일로 남긴다. staged 최종 상태와 undo 포함 이벤트 이력은 매 토글마다 review-feedback draft(memoryActivations.states/events)로 저장되고, 제출 전 재진입 시 복원해 overlay effect가 서버 데이터 위에 다시 입힌다. 제출된 리뷰와 admin viewAs는 복원하지 않는다.
- 서버: review-feedback POST가 memoryActivations를 draft에 보존하고, 최초 제출(기존 submittedAt 없음)일 때만 states를 메모리 문서에 일괄 적용한 뒤 events 전체를 memoryActivationLogs에 기록한다(applied/appliedAt로 실제 적용 이벤트 구분, toggledAt 기반 문서 ID로 재시도 멱등). 적용은 submittedAt 기록 전에 수행해 실패 시 재제출이 가능하다. 비활성화 이유 없는 state는 적용에서 제외한다.
- 클러스터: 재활성화 토글마다 /api/memory/session-clusters의 preview 모드가 staged 활성/비활성 가정(assumeActive/InactiveMemoryIds)으로 after snapshot을 계산해 반환하고, 클라이언트가 화면 snapshot만 갈아끼운다 — Firestore snapshot/memory 문서는 쓰지 않는다. preview는 토글 직후와 draft 복원 재진입을 한 effect로 처리하고, staged 비활성화만으로는 재계산하지 않는다(화면 필터로 충분). 재활성화가 적용된 제출 직후에는 실제 after snapshot 재생성을 best-effort로 1회 호출해 persist한다.
- 제거: /api/memory/active-state 라우트 삭제(호출자 없음). 15.309의 로그 스키마는 toggledAt/applied/appliedAt 필드가 추가된 형태로 유지된다.

### 15.311 Admin review answers tab shows memory activation toggles `[implemented 2026-07-20]`
...
### 15.315 Unify cluster input population for snapshots and full clustering `[implemented 2026-07-21]`

- 배경: 세션 리뷰 overlay(미션별 before/after snapshot)와 /agent 메모리 뷰(최신 full 클러스터 캐시)의 클러스터가 서로 다르게 보였다. 알고리즘은 generateClusterGraph 하나를 공유하지만, snapshot 입력은 missionOrder 기반 누적 필터를 타는 반면 full 입력은 필터 없이 전체 활성 메모리 최신 300개를 사용해 모집단부터 다를 수 있었다. mean-centering과 pairwise 분위수 threshold가 입력 집합 전체의 함수라 작은 입력 차이도 클러스터 경계를 통째로 바꾼다.
- 실데이터 확인(2026-07-21, 참가자 12명/메모리 255개, read-only 스크립트): source.missionId 없는 메모리 0개, missionOrder 밖 미션 메모리 0개, 유저당 최대 총 62개(활성 55)로 300 상한 절단 사례 0. 스키마도 전부 0.1.2 계열로 레거시 없음. 즉 현재 데이터에서 입력 집합 차이는 이론상이며, 실제 보이는 불일치는 생성 시점(동결 snapshot vs 이후 재생성된 최신 캐시)과 /agent 세션 필터가 최신 캐시를 클라이언트에서 node만 걸러 보여주는 표시 방식에서 온다.
- 수정: loadClusterInputItems를 memoryItems에서 memoryClustering으로 이동해 snapshotItems와 나란히 두고, 공용 isClusterScopeMission(온보딩 또는 missionOrder 내 미션) 필터를 적용했다. self/admin clusters GET/POST는 profile의 missionOrder를 로드해 전달한다. 클러스터링 입력 상한 MAX_ITEMS(300)는 full/snapshot 양쪽에서 제거해 스코프 안 활성 메모리 전체가 입력된다(15.290의 300 상향을 대체). 기존에 저장된 snapshot 문서와 제출된 리뷰 응답은 어떤 경로로도 다시 쓰지 않는다.
- 남은 작업(다음 단계): (1) /agent 세션 필터가 최신 캐시 node 필터 대신 해당 미션의 동결 after snapshot을 그대로 표시, (2) 리뷰 제출로 활성 토글이 반영될 때 full 캐시 동기화 방식 결정, (3) label/colorIndex 승계 계보 일원화. → 15.316에서 (1)(3) 구현, (2)는 최신 캐시가 표시 경로에서 빠지면서 retrieval evidence 전용이 되어 별도 동기화 불필요로 종결.

### 15.316 Memory view renders frozen after snapshots per session `[implemented 2026-07-21]`

- 배경: 15.315의 입력 통일 후에도 종익 1일차 데이터에서 세션 리뷰(after snapshot, 오후 4:36 생성, 5 clusters·7 edges)와 admin 메모리 뷰(최신 full 캐시, 오후 11:30 생성, 온보딩 칩 node 필터 후 2 clusters·16 edges)가 같은 12개 노드에 다른 클러스터를 보여줬다. 메모리 뷰의 세션 칩이 재클러스터링이 아니라 하루 마지막 세션 종료 시점의 41개 전체 클러스터링을 클라이언트에서 node만 걸러 보여줬기 때문 — 분위수 threshold와 mean-centering이 41개 코퍼스 기준이라 경계/edge가 다르다.
...
### 15.318 Before snapshot equals previous session end; profile memories appear from after `[implemented 2026-07-21]`

- 배경: 이상준 데이터에서 다음 세션(102001)의 세션 이전 탭과 온보딩의 세션 이후 탭이 달랐다. 원인은 두 가지 — (1) before 스냅샷 입력이 설계상 이번 미션의 사전정보 파생 메모리를 포함해(온보딩 9 + 사전정보 3 = 12개) 전역 재클러스터링이 일어나고, (2) 리뷰 overlay의 세션 이전 탭은 promoted 판정(source.missionId == 현재 미션, 사전정보도 포함됨)으로 그 3개를 숨겨 화면엔 9노드만 보였다. 같은 9개처럼 보이는데 묶임이 달라 혼란을 만들었다.
- 결정: 사전정보 파생 메모리는 세션 이후(after) 스냅샷부터 보이게 한다. snapshotItems의 before 페이즈가 이번 미션 메모리를 sourceType 구분 없이 전부 제외하도록 변경 — before(N) 입력이 after(N-1)과 동일해지고, 결정적 파이프라인 + 15.316 승계(동일 item set 라벨/색 재사용)로 before(N)은 after(N-1)과 동일하게 렌더된다.
- 어긋나지 않는 것들: 과거 저장된 before 스냅샷(사전정보 포함 규칙)은 불변 원칙대로 재생성하지 않고 그대로 둔다. 세션 이전 탭의 promoted 숨김 필터는 과거 스냅샷 표시 호환을 위해 유지한다(새 스냅샷에선 자연히 no-op). 세션 이전 탭의 원래 입력한 내용 섹션(사전정보 원문)은 그대로 표시한다. retrieval은 스냅샷과 무관하므로 사전정보 메모리는 세션 중 검색에 계속 참여한다.
- 알려진 희귀 예외: 세션 N 종료의 중복 아카이브가 이전 메모리를 비활성화하면 before(N)이 after(N-1)과 노드 1개 차이 날 수 있다(cosine 0.92 이상 중복일 때만).

### 15.319 Regenerate after snapshot on any activation change at submit `[implemented 2026-07-21]`

- 배경: 15.318 배포 후에도 이상준의 세션 이전(102001)과 온보딩 세션 이후가 여전히 달랐다. 두 원인 — (1) 기존 저장 스냅샷은 동결이라 새 규칙이 소급되지 않음(설계대로), (2) 실제 버그: 리뷰 제출 시 after snapshot 재생성이 hadReactivation(재활성화 존재)일 때만 돌아서, 비활성화만 2건 제출한 이상준의 after(102001)가 비활성 2건을 포함한 38개짜리 클러스터링(12:06 생성) 그대로 남았다. 15.310의 서술(제출 시 재생성)과 구현이 어긋난 스펙 불일치.
...
### 15.322 Idle decay becomes an always-on target-curve controller `[implemented 2026-07-21]`

- 배경: 종익 메모리에서 직접 비활성 외에 weight 0.5 미만이 하나도 없었다. 기존 decay는 활성 수가 cap을 넘어야만 도는 게이트라 스터디 규모(활성 73 < cap 78)에서 retrieval 65턴 내내 0건이었고, 사용자의 원래 의도는 cap이 하락 시작점이 아니라 상시 하락의 결과로 삭제가 발생하기 시작하는 균형 풀 크기(total 100→80-90, 200→110-120, 300→120-140)였다.
- 수정: idleDecayWeightLoss를 상시 컨트롤러로 변경 — 활성 ≥ 목표면 기존 0.006→0.012 램프 유지, 활성 < 목표면 0.006 × 활성/목표 비례 감속으로 항상 0보다 크게. 목표 곡선(activeMemoryCap)은 사용자 제시 체크포인트를 이미 인코딩하고 있어 그대로 둔다. retrieval 로그의 totalMemoryCount/activeMemoryCap/idleDecayWeightLoss/idleDecayDeltas 필드도 그대로라 분석 시 속도 추적 가능.
- 효과 예시: 종익(활성 73, 목표 78) 기준 미사용 메모리 턴당 -0.0056, 세션(~17턴)당 약 -0.095 — 한 세션만 안 쓰여도 50% 미만이 화면에 보인다. 미사용 0.5 메모리 소멸까지 ~89턴(4-5세션), 목표 초과 포화 시 0.012로 ~42턴. retrieval 보상 +0.04는 decay 약 7턴을 상쇄해 계속 쓰이는 메모리는 유지된다. 초기 소규모 풀(활성 10, 목표 70)은 턴당 -0.00086으로 거의 잊지 않는다.
- 유의: 스터디 중간 forgetting 조건 변경(사용자 확정). 기존 weight 기록은 유지되고 이후 턴부터 새 속도가 적용된다. 이제 매 턴 비검색 활성 메모리 전수에 weight patch가 발생하지만(유저당 ~70건 병렬 PATCH) 스터디 규모에서 지연·비용은 무시 가능하다.
### 15.323 Retune decay rates 1.5x and backfill eligible participants `[implemented 2026-07-21]`

- 배경: 15.322의 0.006/0.012로는 미사용 0.5 메모리 수명이 83턴(~5세션)이라 첫 사멸이 스터디 후반에나 나타나고, 유입 ~1.1개/턴 기준 평형 활성 풀이 목표+15~20에 잡혀 total 100에서 활성 80-90이 달성되지 않는다는 지적(사용자).
- 수정: IDLE_DECAY_WEIGHT_LOSS 0.009, IDLE_DECAY_MAX_WEIGHT_LOSS 0.018로 정확히 1.5배 상향(공식·램프 포화 +50·비례 감속 구조는 동일). 미사용 0.5 수명 ~56턴(3.5세션), 첫 자연 사멸이 total 80-100 부근에서 발생, 평형은 80대 중반에서 목표 곡선을 추종. 좋아요/싫어요 ±0.04는 decay 4~9턴치로 여전히 완만.
- 데이터 backfill: 완료 세션 2회 이상 참가자 전원의 활성 메모리에 새 속도 기준 로그 재생 추정 감쇠를 적용. 턴별로 라이브에서 이미 적용된 idleDecayDeltas와 종익의 기존 0.006-기준 backfill 적용분은 차감해 중복 계산을 막고, 바닥 0.05 클램프로 비활성 전환은 만들지 않는다. user_disabled/비활성 제외. 백업은 exports/decay-backfill-*에 사용자별 원본 weight 포함.

### 15.324 Lower target curve and sqrt slowdown so deaths begin near 70 `[implemented 2026-07-21]`

- 배경: 15.323 이후에도 종익(활성 73, 총 87)의 최저 weight가 0.29에 머물렀다. 원인 두 가지 — (1) 기존 곡선은 총 70개까지 목표 70이라 활성이 목표를 넘기 어려워 압박이 늦게 걸리고, (2) 목표 미만의 선형 비례 감속이 초기 소규모 풀에서 감쇠를 거의 0으로 만들어 소멸 파이프라인이 총 70개 규모까지 차지 않았다. 사용자 요구: 총 70개쯤부터 사라지기 시작, 100개→활성 80, 200개→120 유지, 데이터 수정 없이 로직만.
- 수정(곡선): activeMemoryCap을 40 + 0.4 × total 선형(고정점 100→80, 200→120 정확 통과)으로 낮추고, 200 이후는 값·기울기가 이어지는 제곱근 구간(성장 4, 오프셋 160; 300→~142)으로 완만화.
- 수정(감속): 목표 미만 구간을 0.009 × √(활성/목표)로 변경 — 작은 풀에서도 감쇠가 유의미하고(예: 총 30/활성 25 → 턴당 0.0062, 기존 0.0043) 목표 접근 시 base에 수렴한다.
- 예상 궤적(유입 ~17개/세션 기준): 세션1 미사용 메모리는 총 ~70 시점에 weight 0.1~0.2로 소멸 직전 구간에 진입하고 첫 자연 소멸은 총 80~95 부근. base 0.009 상한 때문에 총 70에서 즉시 소멸까지는 수학적으로 불가(0.5/45턴=0.011 필요) — 더 앞당기려면 base 상향이 필요하다.
- 데이터는 변경하지 않음(15.323 backfill 상태 유지). 기존 로그의 activeMemoryCap 필드는 기록 당시 곡선 값이므로 분석 시 세대 구분 필요.

### 15.325 Reset JONGIK to post-onboarding; early-end confirm and button wording `[implemented 2026-07-21]`

- 데이터 리셋(사용자 지시): 종익이 실험 시간을 준수하지 못해 온보딩 제외 전체 미션을 재진행하기로 함. 비온보딩 세션 5개(+drafts/reviewTurns), 비온보딩 메모리 75건, 오염 캐시 5건, 비온보딩 스냅샷 10건, 리뷰 피드백 5건, activation 로그 16건, profile_memories 5건(+revisions), retrieval 로그 55건을 백업 후 삭제. 온보딩 메모리 12건은 온보딩 리뷰 직후 상태로 복원 — 활성 10건은 weight 0.5/retrievedCount 0/lastRetrievedAt null, 온보딩 리뷰에서 비활성화한 2건은 비활성 유지. 온보딩 세션·리뷰·스냅샷 pair·온보딩 캐시는 보존. 백업: exports/reset-jongik-non-onboarding-20260721-225200.
- 조기 종료 확인: 온보딩 제외 미션에서 미션 종료 클릭 시 경과 시간이 제한 시간(missionDurationMinutes, 기본 30분) 미만이면 아직 N분이 지나지 않았습니다 확인 스트립을 먼저 띄운다. 확인하면 기존 최종 디자인 미선택 확인으로 이어지고, 취소하면 계속 진행한다.
- 워딩: 종료 버튼을 세션 종료에서 미션 종료로 변경(세션 종료됨 → 미션 종료됨 포함). 버튼을 지칭하는 안내 문구, 종료 실패 에러 메시지, 튜토리얼 2개 스텝 및 Final Design 스텝 본문도 함께 갱신.

### 15.326 Session timer pauses outside the session page `[implemented 2026-07-21]`

- 요청: 참가자가 세션 중 로비로 나가면 타이머가 멈추고, 다시 들어오면 이어서 작동해야 한다.
- 설계: 벽시계(timerStartedAt 고정) 모델을 일시정지 누적(fold) 모델로 전환. 세션 문서에 timerElapsedMs(확정 누적)·timerStartedAt(진행 구간 시작, 멈춤이면 null)·timerFirstStartedAt(최초 시작 벽시계, meta 보존용)을 저장한다. 모든 스냅샷 저장이 열려 있는 구간을 누적에 접고 구간 시작을 저장 시각으로 갱신하므로, 어떤 방식으로 이탈해도(브라우저 강제 종료 포함) 손실은 마지막 저장 이후 구간뿐이다.
- 일시정지/재개: 세션 컴포넌트 unmount(로비 이동 등 라우트 전환)와 pagehide에서 pauseTimer 저장으로 구간을 닫고, 재진입 로드가 timerElapsedMs에서 새 구간을 열어 재개한다. 탭 전환(visibility hidden)은 기존 flush(진행 유지 체크포인트)만 수행해 타이머가 계속 흐른다 — 다른 탭에서 레퍼런스를 보는 시간은 세션 시간에 포함된다. admin viewAs는 재개하지 않고 동결 표시한다.
- 파생 갱신: 카운트다운 표시·1분 미만 빨간 강조·조기 종료 확인(15.325)·sessionMeta timerElapsedSeconds가 모두 총 경과(누적+진행 구간) 기준으로 바뀌었다. mission-progress의 시간 초과 판정도 벽시계 대신 총 경과를 쓰고(진행상태 projection에 timerElapsedMs 추가), 완료 세션은 완료 시점 누적으로 고정 표시한다(timerEndedAt state 제거). setup 스킵 판정과 hasSessionStarted에 누적 경과 신호를 추가했다.
- 호환: timerElapsedMs가 없는 legacy 문서는 로드 시 벽시계 경과(완료 세션은 endedAt-timerStartedAt)로 1회 환산된다. 문서의 timerStartedAt 의미가 진짜 시작 시각에서 마지막 저장 시각으로 바뀌므로, 세션 시작 벽시계가 필요한 분석은 timerFirstStartedAt을 쓴다.

### 15.327 Admin card timing uses accumulate-model fields `[implemented 2026-07-22]`

- 증상: 15.326 배포 후 새로 완료된 세션(종익 303001 재진행, 따란 301001)이 관리자 유저 카드에서 시작·소요 시간이 표시되지 않았다. 누적 모델에서 완료·일시정지 세션의 timerStartedAt이 null인데 카드가 여전히 timerStartedAt-endedAt 벽시계 차이로만 계산했기 때문.
- 수정: 진행상태 projection과 MissionProgress에 timerFirstStartedAt을 추가하고, 카드 표시를 시작=timerFirstStartedAt(레거시는 timerStartedAt 폴백), 소요=timerElapsedMs 누적(진행 중이면 마지막 저장 이후 구간 가산, 레거시는 기존 벽시계 차이 폴백)으로 전환. 완료는 소요, 미완료는 경과 라벨.

### 15.328 Reset Geumjin 303001 and lock mission end until two minutes before limit `[implemented 2026-07-22]`

- 데이터 리셋(사용자 지시): 이금진의 mission-20260611-303001이 13분 만에 종료되어 진행 전으로 리셋. 세션 문서+drafts 10+reviewTurns 8, during 메모리 10건, 오염 캐시 1건, 스냅샷 pair, 제출된 리뷰 피드백, 사전정보 문서(+revisions), retrieval 로그 8건을 백업 후 삭제하고, 그 세션이 건드린 타 미션 메모리 27건의 weight/retrievedCount/lastRetrievedAt을 로그 기반으로 복원(이후 활동 없음 확인). latest 캐시는 직전 세션(203001) 시점으로 복귀. 백업: exports/reset-geumjin-303001-20260722-143840.
- 종료 잠금: 온보딩 제외 미션에서 미션 종료 버튼을 제한 시간 2분 전(missionDurationMinutes-2, 30분 미션이면 28분)까지 disabled 처리. 잠금 해제 후 제한 시간 전 클릭은 기존 15.325 조기 종료 확인이 그대로 적용된다. 경과 판정은 일시정지 누적 모델 총 경과 기준이며 타이머 표시가 매초 리렌더해 함께 갱신된다. 튜토리얼 미션 종료 스텝 문구도 갱신.

### 15.327 Fix localhost mission asset URLs and budget the OpenAI asset fallback `[implemented 2026-07-22]`

- 배경: 2026-07-21 23:44/23:48 수연(nsy0609)의 Luna(mission-20260611-101001) 신규 목업 생성 2회가 Vercel `FUNCTION_INVOCATION_TIMEOUT`(icn1, maxDuration 180s 도달)으로 실패. 조사 결과 6개 미션(101001/102001/103001/201001/202001/303001)의 `assetImages[].url`이 `http://localhost:3000` origin으로 저장돼 있었고(어드민이 로컬에서 업로드), `isPubliclyReachableAssetUrl`이 이를 거부해 프로덕션의 asset-led 신규 생성이 항상 Stitch를 건너뛰고 OpenAI direct HTML fallback(비스트리밍 full-HTML, 최대 2회 순차)으로만 동작해 왔다. 이 폴백은 180s를 넘기면 플랫폼 타임아웃(수연), 시간 안에 끝나도 coverage 미달 실패 — sally01323 201001 3회 (3/6, 4/6), charlie 102001 (0/2)·202001 (5/7) — 로 나타났다. 같은 Luna 미션에서 yeeunhan은 레퍼런스 인용 image-led 생성이라 asset 분기를 우회해 실제 Stitch 스크린 생성에 성공했다.
- 데이터 수정(2026-07-22 적용, 배포 불필요): 위 6개 미션의 asset url 25건을 `https://vibe-design-agent.vercel.app` origin으로 교체. 교체 전 각 URL의 프로덕션 200 image/png 응답과 path 쿼리-저장 path 일치를 검증했고, 적용 후 재조회로 9개 미션 전부 정상 host를 확인했다. 이제 해당 미션의 신규 목업은 실제 Stitch asset-led 경로(`generate_screen_from_text` + coverage 검증)를 탄다. `/api/mission-assets` GET은 무인증 공개라 Stitch/iframe 접근에 문제 없다.
- 코드(`/api/stitch`): `maxDuration` 180→300. POST 진입 시 `routeDeadlineAt`(+270s)을 계산해 `generateOpenAiAssetFallbackScreen`에 전달하고, 폴백 재시도(no-markup/coverage 미달)는 잔여 예산이 90s 이상일 때만 수행하며, 각 completions 호출에 잔여 예산 timeout(최소 60s)을 건다. 예산 소진 시 플랫폼 강제 종료 대신 기존 명시적 에러 메시지로 실패한다.
- 데이터 리셋(사용자 지시, 2026-07-22): 실패 세션은 부분 정리(시안 2·3, 실패 턴 메시지/로그/draft/reviewTurn 삭제) 후, 벽시계 기준 671분 시간 초과 상태라 결국 종익(15.325)과 같은 방식으로 진행 전 상태로 풀 리셋했다 — 세션 문서(+memoryDrafts/reviewTurns), profile_memories(+revisions), before-session 메모리 5건, Luna retrieval 로그 3건, 오염 클러스터 캐시 1건을 백업 후 삭제하고, Luna 턴이 건드린 온보딩 메모리 7건을 온보딩 리뷰 직후 상태(weight 0.5/retrievedCount 0/lastRetrievedAt null)로 복원. 온보딩 세션·리뷰·스냅샷 pair는 보존. 백업: exports/backup-nsy0609-luna-failed-turns-20260722-104347.json(부분 정리 전), exports/reset-nsy0609-luna-20260722-105043.json(풀 리셋 전). 수연은 재입장 시 setup(사전 정보 입력)부터 30분 전체로 다시 시작한다.

### 15.328 Retry intermittent Stitch INVALID_ARGUMENT and promote board-less EDIT_MOCKUP to generation `[implemented 2026-07-22]`

- 배경: 2026-07-22 하루에만 invalid argument 실패 3건 — redannn 온보딩 11:29 `create_project`(인자가 제목뿐인 고정 호출), matjisloth Verde(103001) 16:25 `generate_screen_from_text`(325자 한국어), jaeryung 온보딩 16:33 `generate_screen_from_text`(6,272자 영어, 30초 뒤 5,964자 유사 프롬프트로 성공). 같은 날 더 긴 프롬프트(6,578자/10,996바이트)가 성공했고 실패 사이사이 다른 생성은 계속 성공 — 요청 내용 문제가 아니라 Stitch 백엔드가 간헐적으로 INVALID_ARGUMENT를 반환하는 것으로 판정(실험 기간 중 이 오류는 이날 처음 등장). 세 건 모두 수동 재시도로 회복됐다.
- 서버(`/api/stitch`): `withInvalidArgumentRetry` 추가 — invalid argument 시 8~10초 backoff 후 1회 재시도. 적용 지점은 create_project 3곳(초기/replacement/API-key fallback)과 비-asset-led `generate_screen_from_text` 2곳(plain 생성 초기/replacement). asset-led generate는 invalid argument가 OpenAI 폴백 신호(15.262)이므로 재시도 없이 기존 계약 유지.
- 클라이언트(main page): matjisloth 케이스에서 드러난 라우팅 빈틈 수정 — `[EDIT_MOCKUP]`인데 edit 대상 artboard가 전혀 없으면(방금 fork된 빈 시안; 15.253/15.254의 synthetic 가드로는 안 잡힘) screenId 없는 요청이 서버에서 수정 지시문만으로 신규 생성되던 것을, isNew 승격으로 바꿔 정식 생성 경로(brief/디자인 스타일 게이트, 스타일 이미지 image-led, mission asset asset-led 포함)를 태우고 채팅에 승격 안내를 붙인다. 승격 판정은 아래 edit 경로와 동일한 target lookup(selectedElementBoard → activeArtboardId → 현재 시안 마지막 board)을 사용해, lookup이 board를 찾는 기존 edit 동작은 그대로 둔다.
- 미해결(별도 과제): 사용자가 첨부한 사진을 기존 목업에 콘텐츠 이미지로 실제 삽입하는 경로는 여전히 없다(스타일 참조 생성과 텍스트 기반 로컬 편집뿐). matjisloth 대화의 근본 불만("이미지가 안 들어갔는데")은 이 갭에서 온 것. 첨부 이미지를 Storage에 올려 URL로 주입하는 방식 검토 필요. 부수 관찰: 실패 라벨이 edit에도 항상 목업 생성 실패로 뜨는 stale closure 표시 버그가 있음(catch가 render 시점 mockupOperation을 읽음) — 이번엔 수정하지 않음.

### 15.329 Keep session doc under the 1MiB limit by trimming message image payloads `[implemented 2026-07-22]`

- 인시던트: 2026-07-22 18시 종익(302001, 이커머스) 세션에서 미션 종료 및 메모리 확정에 실패했습니다 토스트 반복. 실측 결과 세션 문서가 1,051,738 bytes로 Firestore 1MiB(1,048,576B) 한도 초과 — messages 필드가 770KB였고, 그중 사용자 첨부 스타일 이미지의 base64 dataUrl(`styleImage`) 337KB × 2턴 = 675KB(문서의 64%), URL 캡처 preview(`stitchReferenceImage`) 38KB. 18:41:22 마지막 저장 성공 이후 모든 setDoc이 거부돼 자동저장이 조용히 실패했고(참가자 인지 불가, 채팅/draft는 subcollection이라 계속 동작), `completeSession` 첫 단계 `persistSessionSnapshot()`이 던지면서 종료가 불가능해졌다. artboard HTML(77KB)과 activityLog previousHtml(93KB, trim budget 미도달)은 원인이 아니었다.
- 수정(클라이언트): ① `trimMessagesForSave` — 저장 페이로드의 message에서 `styleImage`/`stitchReferenceImage` 필드를 제거(메모리 상태는 유지해 현재 방문 중에는 계속 표시, 재입장 후에는 미표시). setDoc merge에서 messages 배열은 통째로 교체되므로 잔존 없음. ② 스냅샷 setDoc 실패를 try/catch로 잡아 60초 스로틀 토스트(세션 저장에 실패했습니다...)로 노출하고 boolean을 반환. ③ `completeSession`은 스냅샷 저장이 명시적으로 실패하면 즉시 실패 처리해 finalArtboardId가 저장 안 된 채 draft 승격이 진행되는 것을 막는다.
- 라이브 복구 절차(종익): 배포 완료 후 새로고침 — 문서는 18:41 상태로 로드되지만 새 코드의 다음 저장이 dataUrl을 strip해 ~380KB로 내려가 저장이 재개된다. 18:41 이후 화면상 채팅 턴은 사라지나 해당 턴들의 memory draft(18:49까지)는 서버에 보존돼 메모리 확정에는 손실이 없다. 서버측 문서 수술은 불필요(클라이언트가 전체 상태를 다시 쓰므로 효과 없음).
- 잔여 리스크: 이미지 첨부 없이도 메시지/로그가 매우 긴 세션은 이론상 한도에 접근할 수 있다 — 이제 저장 실패가 토스트로 보이므로 재발 시 즉시 인지 가능. 근본적으로 첨부 이미지를 Storage로 옮기는 과제는 15.328 미해결 항목과 함께 검토.

### 15.330 Split review Part 1 into two steps (Likert then freeform) `[implemented 2026-07-24]`

- 문제: 세션 리뷰 Part 1 패널이 Likert 3문항(가로 3열)과 자유응답 1문항을 한 번에 전체폭 strip으로 표시해, 그 아래의 작업/세션 화면이 가려 잘 보이지 않았다.
- 변경: `MemoryReviewIntroPanel`(`src/app/main/[missionId]/page.tsx` 인라인)에 내부 스텝 상태를 추가해 Part 1을 두 스텝으로 나눈다 — 스텝 1은 Likert 1~3만, 스텝 2는 자유응답 4만 표시한다. 헤더 우측에 현재 스텝/2 표시. 스텝 1 다음 버튼은 Likert 3개가 모두 선택돼야 활성화되며 스텝 2로 넘어가고(서버 저장 없이 로컬 전환), 스텝 2에는 이전 버튼과 다음 버튼을 두어 이전은 스텝 1로 돌아가고 다음은 기존 `submitIntro`를 그대로 호출해 Part 1 답변 4개를 draft 저장한 뒤 Part 2 overlay로 진입한다. 문항 텍스트/키/저장 페이로드(`MEMORY_REVIEW_INTRO_KEYS` 4개)와 Part 2 진입 계약은 변경 없음. admin viewAs는 종전대로 Part 1을 건너뛴다.

### 15.331 Persist memory review overlay intent in the URL to survive remounts `[implemented 2026-07-24]`
...
### 15.333 Require memory review submission to unlock the next mission `[implemented 2026-07-24]`
...
### 15.335 Allow up to three attached style images with size guards `[implemented 2026-07-24]`

- 요청: 채팅 입력 이미지 첨부가 1장뿐 — 3장까지 확장하고, 15.329(첨부 base64로 세션 문서 1MiB 초과, 저장 영구 실패) 재발이 없는 선에서 크기 제한을 강화.
- 클라이언트(`src/app/main/[missionId]/page.tsx`, `chat-input.tsx`, `chat-bubble.tsx`): 첨부 state를 배열(최대 3장)로 확장. 첨부 파이프라인은 기존 12MB 입력 제한 + 1280px 다운스케일(jpeg 0.85)에 더해, 결과 dataURL이 70만 자를 넘으면 0.7로 재압축하고 150만 자(~1.1MB)를 넘으면 첨부 자체를 거부한다(toast) — 세션 문서는 trimMessagesForSave가 계속 지키고(styleImages 필드도 제거 대상에 추가), 이 가드는 채팅 vision/레퍼런스 분석/Stitch 요청 페이로드가 3장 합산으로 서버 한도(chat 8MB, references 장당 5MB)에 닿는 것을 입구에서 막는다. 장수 제한은 비동기 디코드 중에도 ref 거울 + pending 카운터로 정확히 지킨다. alert는 toast로 교체. 파일 선택 input은 multiple, 붙여넣기는 기존대로 한 장씩 append, attachment tray는 장별 칩과 개별 제거. 채팅 bubble은 여러 장을 렌더한다(legacy 단일 styleImage 폴백).
- 소비 계약: 첫 장(primary)이 기존 단일 계약을 그대로 탄다 — Stitch 이미지 주도 재구성 입력, memory draft의 image source, previousImageContext carry-forward, message.styleImage 하위호환 필드. 다중의 실효는 vision 소비처 두 곳: `/api/chat`은 styleImageContext.dataUrls(최대 3, 각각 기존 패턴/8MB 검증)를 받아 첫 장 detail high + 나머지 low로 최종 응답 provider에 주입하고(OpenAI/Anthropic 모두), 시스템 프롬프트(chatAttachedStyleImagePrompt)가 장수와 primary 규칙을 안내한다. `/api/references`는 styleImages 배열(각각 5MB 검증)을 받아 vision 스타일 단서 분석이 여러 장의 공통 방향을 한 문구로 합친다. 두 라우트 모두 단일 필드로 폴백해 구버전 페이로드와 호환.
- 부수: 1~9장 4.4의 /api/chat 이미지 서술이 15.301(dataUrl 직접 vision 주입) 이후 stale인데 마킹이 누락돼 있던 것을 이번에 함께 마킹.

### 15.336 Prevent staged memory toggle loss on remount and block Chrome auto-translate crashes `[implemented 2026-07-25]`

- 버그 제보(Nam Suyeon 사례): 리뷰에서 메모리 여러 개를 비활성화하던 중 전면 에러 화면(새로고침 안내)이 떴고, 복구 후 오늘 생성된 메모리 1개의 토글만 남고 나머지 staged 토글이 전부 사라짐. 크롬 자동번역을 켠 상태였음.
- 원인 분석(2026-07-25, 두 결함의 연쇄): ① 크래시 — Chrome 자동번역이 React 관리 DOM을 font 태그로 변조, 비활성화 클릭의 대형 리스트 재구성 커밋에서 removeChild 계열 uncaught 예외 → 에러 바운더리가 없어 전체 트리 언마운트(pagehide flush 리스너도 제거). ② 데이터 소실 — review-feedback POST가 memoryActivations를 merge 없이 통째 교체하는데, 리마운트 직후 draft 복원(GET) 완료 전에도 mreview URL 파라미터로 오버레이가 열려 토글이 가능했음. 이 창에서 토글 1개를 누르면 그 1개만 담긴 payload가 서버 draft의 기존 staged 토글 전체를 덮어씀. 번역은 크래시의 원인이고, 소실 자체는 번역 없이 새로고침만으로도 재현 가능한 별도 결함.
- 서버(`src/app/api/memory/review-feedback/route.ts`): mergeMemoryActivations 추가 — states는 memoryId별로 더 최신 결정(state/event toggledAt 최댓값)을 가진 쪽의 표현(항목 또는 부재=baseline undo)을 택하고, 이력 없이 항목만 없는 payload(복원 전 클라이언트)는 결정 시각 0이라 기존 항목을 이기지 못한다. events는 (toggledAt, active, memoryId) 기준 union이라 out-of-order 도착에도 유실되지 않는다. answers의 mergeAnswers와 같은 보호 원리.
- 클라이언트(`src/app/main/[missionId]/page.tsx`): 복원 완료 플래그(memoryReviewRestoreDone) 도입 — 복원 전에는 save/flush payload에서 memoryActivations 필드를 생략(서버가 기존 저장분 유지)하고 detail card의 토글 UI를 숨기며 콜백도 안내 toast와 함께 차단. 복원 최종 실패 시에는 잠금을 풀되 서버 merge가 보호. 제출 시 after snapshot 재생성 여부와 표시 전용 변경 목록은 클라이언트 staging ref 대신 서버 응답의 merge된 states 기준으로 판단(복원 전 제출도 정확). flush는 응답 ok 확인 후에만 dirty 해제·dedupe 키 갱신(실패를 저장됨으로 오인해 영구 미전송되던 결함 수정). 토글 임시 저장 실패 시 error toast 추가(기존엔 무조건 success toast).
- 크래시 방지/복구(`src/app/layout.tsx`, `error.tsx`, `global-error.tsx`): html translate=no + google notranslate meta로 자동번역 차단. 세그먼트/글로벌 에러 바운더리 신설(Next 16.2 unstable_retry 사용) — 다시 시도/새로고침 안내와 draft 복원 안내 표시.
- 1~9장 동기화: 3장 공통(root layout/에러 처리) 신설, 3장 리뷰 흐름과 4장 forgetting의 draft 저장 서술에 merge/복원 게이팅 마커 추가.

### 15.337 Harden mission completion against OpenAI outages with idempotent retry `[implemented 2026-07-25]`

- 버그 제보(조수빈 303001 사례): 미션 종료 버튼을 눌러도 `미션 종료 및 메모리 확정에 실패했습니다` 토스트가 반복되며 메모리 리뷰로 진입 불가.
- 원인 분석(2026-07-25): 트리거는 OpenAI 계정/프로젝트 단위 API 장애(18:38:39부터 무료 endpoint 포함 전 호출 500 — 전역 status는 정상, no-auth/garbage-key 대조 프로브는 401. request id `req_2769e708666342899e1fa03facb41c26`). 이를 하드 블록으로 만든 코드 결함 두 개:
  - ① `/api/memory/complete-session`: draft마다 개별 OpenAI 임베딩 호출을 무보호 `Promise.all`로 병렬 실행 — 1개만 reject해도 라우트 전체가 500이고, `maxDuration` 미선언이라 플랫폼 기본 시간에도 취약. 1차 실행이 draft 15개 중 9개만 승격한 채 중단됐고(session status는 active 유지, f2f8ead1은 memory만 쓰이고 draft status 미갱신), 라우트가 재시도 멱등성이 없어 재실행 시 전량 재승격(weight 하드 리셋)이었다.
  - ② `/api/memory/drafts`: completeSession이 종료 때마다 재생성하는 final-design draft의 인코딩 LLM 호출(`gpt-5.4-mini`)이 uncaught — OpenAI 장애 중엔 매 재시도가 여기서 500으로 죽어 complete-session에 도달조차 못 했다. Firestore 타임라인(update_time)으로 확인.
- 수정(`src/app/api/memory/complete-session/route.ts` 전면 재구성):
  - `maxDuration = 120` 선언.
  - 1단계 분류(쓰기 없음): status 기반 멱등 스킵 — promoted draft는 재승격하지 않고 duplicate cleanup 대상에만 포함, skipped_empty는 재평가하지 않음. final-design draft만은 시도 사이에 `finalArtboardId`가 바뀔 수 있으므로 status와 무관하게 항상 현재 선택 기준으로 supersede/promote를 재평가.
  - 2단계 임베딩 배치 1회: draft별 병렬 호출(N회 burst)을 단일 `embedMemoryInputs` 호출로 통합. 배치 실패 시 빈 embedding으로 승격 — retrieval/clustering의 `ensureFreshMemoryEmbeddings`가 같은 텍스트 계약으로 재생성·write-back하는 자가 치유 경로가 있고, 빈 embedding은 similarity 0으로 안전함을 확인.
  - 3단계 draft별 쓰기: worker 패턴 동시성 6 제한 + draft별 try/catch 실패 격리. superseded final-design draft는 status가 다를 때만 patch하고, 이전 시도가 승격해 둔 고아 final memory가 있으면 `archiveReason: final_design_superseded`로 archive(삭제 아님 — 이력 보존, 재선택 복귀 시 promote 경로의 `archivedAt: null`이 자동 해제).
  - 4단계 완료 확정: 실패가 하나라도 있으면 completed 마킹 없이 `promotion_partially_failed` 500을 반환해 재시도가 실패분만 이어서 처리. 응답 shape(`ok/promoted/completedAt/duplicateCleanup`)는 유지.
- 수정(`src/app/api/memory/drafts/route.ts`): final-design 경로에서만 인코딩 LLM 실패를 catch — 사실 기반 fallback(episode = 원문 입력 `최종 디자인 확정: ...`, keywords/semantic 빈 값)과 `encodeError` 마커로 draft를 저장해 미션 종료가 계속 진행되게 한다. 보강 비교 텍스트는 input 필드에 이미 보존. 다른 draft 경로는 기존 throw 동작 유지(해당 턴 메모리 미생성 = 기존과 동일한 조용한 degradation). enrichment(`buildFinalDesignMemoryInput`)와 source normalization은 원래 best-effort라 변경 없음.
- 사전 검증: `enrichReferenceSources`/`normalizeMemorySources`는 무소스 시 LLM 없이 즉시 반환, retrieve는 빈 embedding을 similarity 0 처리(`retrieve/route.ts` 470행 부근) + `ensureFreshMemoryEmbeddings`(294행)로 치유, `archiveDuplicateMemoriesForTargets`는 누락 id에 안전함을 확인.
- 미해결/운영: OpenAI 계정 장애 자체는 코드 밖 — 대시보드 billing 확인 또는 조직/키 교체 필요. 조수빈 303001은 half-promoted 상태로 대기 중이며 세션 doc의 `finalArtboardId`(Design 2)와 1차 승격된 final memory(Design 3)가 불일치 — 참가자 최종안 재확정 후 종료 재시도. 배포 전 재시도는 구버전 동작(전량 재승격 + 고아 미정리)이므로 배포 후 재시도 권장.

### 15.338 Add inactive-reason filter chips to the inactive memory panel `[implemented 2026-07-27]`

- 요청(인터뷰 준비): 비활성 메모리 그룹 detail panel의 설명 문구("유사 메모리 정리, weight 0 도달, 사용자 설정으로...") 아래에서 비활성화 사유별로 카드 목록을 필터링할 수 있어야 한다. 노출 범위는 `/agent`와 admin 메모리 보기 전용 — 참가자 세션 리뷰에는 보이지 않아야 한다(후속 지시로 범위 확정).
- 구현(`src/components/memory/memory-cluster-side-panel.tsx` 단일 지점): `inactiveReasonKeyFor` 분류 키(user_disabled > archivedAt/archived action > weight 0)를 신설하고 기존 `inactiveReasonLabel`을 그 위에 재구성해 카드 라벨과 필터 판정이 드리프트할 수 없게 했다. `session-inactive` pseudo-group 선택 시에만 설명 아래 chip 4개(전체/유사 메모리 정리/weight 0 도달/사용자 비활성화)를 건수와 함께 표시하고, 0건 chip은 disabled, 활성 chip 재클릭은 전체 복귀다. 노출은 opt-in prop `showInactiveReasonFilter`로 게이트한다 — `/agent`의 `MemoryClusterPage` 렌더(self·admin 공용 진입점)에서만 켜고, 세션 리뷰 overlay의 렌더는 prop을 넘기지 않아 기본 off다. off일 때는 counts/필터 로직도 동작하지 않아 리뷰 화면 동작이 15.338 이전과 동일하다.
- 상호작용 계약: 그래프 노드 클릭으로 현재 필터에 걸러진 메모리가 새로 선택되면 선택이 이긴다 — prev-selectedMemoryId ref로 "새 선택" 이벤트만 감지해 필터를 자동 해제한다(chip 클릭은 selectedMemoryId를 안 바꾸므로 안 걸림). 그룹/클러스터 전환 시 필터 초기화. scroll-to-selected effect는 필터 해제 다음 렌더에서 재시도하도록 dep을 확장했다. 활성 fallback 카드(클러스터 밖 선택 메모리)는 사유 필터 대상에서 제외한다.
- 적용 범위: 세션 리뷰 overlay, `/agent`, admin 사용자 메모리 페이지 — 셋 다 같은 `MemoryClusterSidePanel`을 렌더하므로 한 곳 구현으로 동시 적용된다.
- 참고: archivedAt 기반은 현재 전부 auto-duplicate(실측 35건)라 "유사 메모리 정리"로 묶었다. 15.337의 final_design_superseded archive도 같은 chip에 떨어지는데 현재 0건이고 기존 카드 라벨("유사 메모리 존재")도 같은 판정이라 분리하지 않았다. 실측 분포(2026-07-27): weight 0 도달 544, 사용자 비활성화 149, 유사 정리 35.

### 15.339 Show full review answers when a single participant is selected `[implemented 2026-07-27]`
...
### 15.341 By-question view: admin excluded, all-participants Likert as average graph `[implemented 2026-07-28]`

- 요청: 답변별 보기에서 전체 참가자일 때 Likert 문항(1·2·3·10)이 모든 사람의 개별 그래프로 나열되지 않고 전체 평균 그래프 하나만 보이게. 관리자는 답변별 보기에서 아예 제외.
- 수정: AverageRatingSessionGraph 추가 — RatingSessionGraph와 같은 10칸 세션 순번 슬롯에 칸별로 해당 순번 세션 응답들의 평균(소수 1자리)을 바로 그리고, 툴팁에 평균·응답 인원, 하단 캡션에 전체 평균과 총 응답 수를 표시한다. 전체 참가자 + rating 문항일 때만 이 그래프로 대체하고(이유 텍스트 포함 개별 나열 생략), 특정 참가자 선택 시에는 기존 개별 그래프·이유를 유지한다.
- 관리자 제외: 답변별 보기의 그룹 목록을 participantGroups만으로 구성하고, 드롭다운에서도 답변별 모드에서는 관리자 항목을 숨기며, 관리자가 선택된 채 답변별로 전환하면 필터를 전체로 되돌린다. 상단 N건 카운트와 빈 상태 판정도 관리자 제외 기준으로 계산한다. 세션별 보기는 기존(하단 관리자 섹션) 유지.

### 15.342 Participant progress strip; hide retired kaithape account permanently `[implemented 2026-07-28]`

- 진행률 스트립: 리뷰 답변 탭 맨 위에 참가자별(P번호 순, 관리자 제외) 진행률을 한 줄 칩으로 표시 — P번호·이름·진행 바·완료 N/전체. 완료 판정은 유저 카드와 동일(온보딩은 프로필 플래그, 나머지는 완료 세션 ∩ missionOrder). missionOrder가 아직 없는 신규 계정은 전체 미션 수로 분모를 대신한다.
- kaithape 영구 숨김: Suyeon Nam의 폐기 계정(kaithape@gmail.com)이 재로그인 때마다 빈 프로필을 재생성해 P8 자리를 차지하는 문제(2026-07-27 재발). 데이터 삭제 대신 코드 레벨 차단 — src/lib/admin.ts의 HIDDEN_ADMIN_VIEW_EMAILS와 isHiddenAdminViewEmail을 추가하고, /api/admin/users가 목록·참가자 번호 부여 전에 제외하며 admin 페이지 병합에서도 방어적으로 거른다. 정상 계정 nsy0609가 P8로 복귀(참가자 20명 기준 P1~P20 검증). 빈 프로필 문서 자체는 남지만 관리자 뷰에는 나타나지 않는다.

### 15.343 Review before phase shows cumulative inactive group `[implemented 2026-07-28]`

- 배경: 종익 마지막 세션 리뷰의 세션 이전 탭에서 비활성 메모리가 0으로 표시됐지만, 실제로는 세션 시작 시점에 리뷰로 비활성화된 메모리 13개(+decay 도달분)가 있었다. overlay 가시성 규칙이 비활성 예외를 after 페이즈에만 적용해 before에서는 항상 0이 되던 표시 규칙 문제 — before(N)=after(N-1) 등식(15.318) 이후로는 같은 상태를 두 탭이 다르게 보여주는 비일관이었다.
- 수정: 비활성 스냅샷-외 예외를 페이즈 무관으로 바꿔 세션 이전 탭에서도 누적 비활성이 after와 같은 규칙으로 비활성 그룹에 dimmed 표시된다. 이번 세션 승격분(promoted)은 before에서 계속 숨긴다. 세션발 비활성 +N 배지는 after 전용으로 유지하고(before는 0), 비활성 필터도 before에서 유효해져 페이즈 전환 시 필터 리셋 대상에서 제외했다. 저장된 스냅샷·리뷰 답변은 불변(표시 로직만 변경).

### 15.344 Session-scoped inactive badge counts real session-caused deaths only `[implemented 2026-07-28]` `[partially superseded → 15.345: 비활성 판정 자체가 snapshot 멤버십 기준으로 바뀌어 배지가 집합 연산으로 정확해짐]`

- 배경: 종익 마지막 세션 리뷰에서 세션 이전 비활성 123 → 이후 125인데 +N 배지가 +121로 표시됐다. 15.299의 세션발 판정 휴리스틱(weight <= 0 전부)이 상시 decay(15.322) 이후 여러 세션에 걸쳐 누적된 사망까지 이번 세션 것으로 세고 있었다.
- 수정: isSessionScopedInactive를 (1) 세션 처리 중 아카이브(sessionArchivedIds), (2) 지금 비활성이면서 세션 이전 스냅샷 입력에 있던 것(직전까지 활성 → 이번 리뷰 비활성화 등), (3) 이번 세션 생성(promoted) 후 비활성이 된 것의 합집합으로 재정의. 세션 중 decay로 죽은 이전 미션 메모리는 before 스냅샷 생성 시점(세션 종료)에 이미 빠져 있어 배지에 포함되지 않는다 — 배지는 리뷰/아카이브가 만든 명시적 변화에 수렴한다. 변화 필터의 같은 휴리스틱 항도 교체.
- 검증(종익 201001): 기존 휴리스틱 119 → 새 정의 +2 (리뷰 비활성화 1 + 완료 시 중복 아카이브 1), 이전 123/이후 125 표시와 정합. legacy before 스냅샷이 없는 세션은 fallback으로 빈 집합을 써 세션 생성분+아카이브만 센다.
- 한계(15.345에서 해소): "지금 비활성" 조건이 현재 문서 상태라 마지막 세션에서만 정확했다. 과거 세션 리뷰에서는 그 세션 before 멤버가 이후 아무 세션에서 죽어도 전부 걸려 +N이 비활성 전체에 수렴했다.

### 15.345 Past session reviews restore phase state from snapshot membership `[implemented 2026-07-29]`

- 증상: 중간 세션의 메모리 리뷰를 나중에 열면 당시 활성이던 메모리들이 비활성으로 표시되고 클러스터가 무너져 보였다(이상준 301001 리뷰: 클러스터 잔존 20개 · 비활성 92 · +92). 세션 이전/이후 두 페이즈 모두.
- 원인: 리뷰 overlay가 클러스터 구조는 동결 snapshot에서 읽으면서 노드별 활성/비활성 판정은 조회 시점 라이브 문서(weight/archivedAt)로 했다. 상시 decay(15.322~324) 이후 현재 상태가 과거 세션 시점과 크게 괴리되어, 당시 활성 멤버가 소급으로 비활성 그룹에 빠지고(클러스터 붕괴·엣지 소실) 15.344의 +N 판정(지금 비활성 ∧ before 멤버)도 이후 누적 사망을 전부 세션발로 셌다. 15.343/15.344가 만든 버그가 아니라 "현재 상태로 과거를 그린다"는 기존 가정이 decay 체제 변경으로 깨진 것이 표시 확장으로 드러난 구조.
- 수정(표시 로직만, 데이터 불변): snapshot 입력은 생성 시점 활성 메모리만 담으므로 itemIds 멤버십이 곧 그 시점 활성 여부의 동결 기록이다. renderSessionImpactGraph의 isInactiveGraphMemory를 스냅샷 존재 시 멤버십 판정(비멤버 = 그 시점 비활성)으로 전환하고, 멤버십이 곧 가시성이 되므로 별도 스냅샷 가드 분기는 제거. itemIds가 빈 구세대 스냅샷은 cluster itemIds 합집합으로 복원. 스냅샷 없는 legacy 세션만 기존 현재 상태 판정으로 폴백. graphItems에 inactiveResolved 플래그를 추가하고 MemoryClusterGraph.isInactiveGraphItem과 MemoryClusterSidePanel.isInactiveMemoryItem이 이 플래그가 있으면 weight/archivedAt 휴리스틱 재판정을 생략한다 — /agent와 admin 메모리 페이지는 플래그를 안 세우므로 기존 현재-상태 동작 유지.
- 파생 효과: +N 배지가 (before ∪ promoted) − after ∪ 세션 아카이브 집합 연산과 동일해져 과거 세션에서도 정확하고, 15.344 검증값(+2)은 시간이 지나도 불변이 된다. staged 재활성화(preview snapshot 교체·비활성 그룹 자리 활성 스타일)와 staged 비활성화(멤버 유지 + 제출 시 반영 배지) 흐름은 그대로다. 최신 세션은 현재 상태 ≈ after 멤버십이라 표시가 바뀌지 않는다.
- 검증(이상준 전 세션, 라이브 Firestore read-only 시뮬레이션): 301001(스크린샷 세션) 구 로직 클러스터 20 · 비활성 92 · +N ~92 → 신 로직 클러스터 106 · 비활성 6 · +N 1. 누적 비활성 궤적은 0→4→5→6→24→55→83→100→118로 세션별 역사 그대로. 최신 세션(303001)은 구/신 동일(143 · 118 · +1)로 무회귀 확인.
- 남은 한계: detail card의 weight %·비활성 사유 텍스트는 여전히 현재 문서 값이다(시점별 weight/사유는 저장하지 않음 — 필요하면 memoryActivationLogs·retrieval log 재생으로 후속). 노드 반지름도 현재 weight 기준.

### 15.346 Asset inclusion overrides style image-count limits in mockup generation `[implemented 2026-07-30]`

- 증상(조수빈 203001, 세션 중 발생): 신규 목업 생성이 목업 생성 실패: OpenAI HTML fallback did not preserve every mission asset (5/6) 로 연속 2회 실패. 몇 분 뒤 3번째 시도는 성공해 세션은 이어졌다.
- 원인: 미션 asset 계약(6개 전부 img src 포함, 미달 시 하드 실패)과 참가자가 확정한 디자인 방향의 정면 충돌. 조수빈의 디자인 스타일에 보조 이미지는 1-2장만, 이미지 3장 이상 콜라주 금지가 명시돼 있고 GENERATE_MOCKUP payload도 one dominant image + 1-2 supporting을 요구 — 모델이 이미지 수 제약을 우선해 asset 1개(유사 프로필 3장 중 마지막)를 확률적으로 떨어뜨렸다. 실제 브리프+스타일로 로컬 재현 시 호출당 약 50% 누락(2회 중 1회 5/6). 인프라 배제: asset 6개 전부 프로덕션 host HTTP 200, localhost 재발/토큰 mangling 아님. 이 미션은 Stitch 본선 coverage가 상습 실패해 완료 세션 14개 대부분이 openai-asset-fallback id로 생성됐다 — 폴백 주사위를 매번 던지는 구조였다.
- 수정(src/app/api/stitch/route.ts): ① 공통 ASSET_PRIORITY_INSTRUCTION 신설 — asset 전부 포함이 브리프/스타일의 이미지 수 제한보다 우선하고, 제한이 있으면 나머지 asset을 작은 보조 썸네일 스트립/아카이브 행/푸터 갤러리로 포함하라는 지시. Stitch URL 본선(assetImageUrlFallbackPrompt), repair(assetImageRepairPrompt), OpenAI 폴백(assetImageTokenFallbackPrompt)과 폴백 system 프롬프트에 모두 주입. ② assetHtmlCoverage가 빠진 asset의 index+note를 반환하고, 폴백 재시도 피드백이 전체 목록 대신 실제 누락 token만 지목. ③ 폴백 시도 2→3회(기존 hasRetryBudget 시간 예산 가드가 각 재시도를 게이트하므로 FUNCTION_INVOCATION_TIMEOUT 노출 없음).
- 검증: 조수빈의 실제 브리프+스타일로 강화 프롬프트 4회 호출 전부 6/6 (기존 프롬프트는 2회 중 1회 5/6). tsc 통과.
- 참고: 옵션 C(미션 asset 축소)는 이미 14명이 6개 조건으로 완료해 비교성 훼손으로 기각(사용자 결정). 게이트 완화(경고+통과)도 에셋 보존 계약 유지를 위해 채택하지 않음.
