...
- `/agent` cluster UI는 좌측에서 cluster list → detail panel → graph 순서로 배치한다. cluster list는 main 세션 리뷰와 동일한 `MemoryClusterList` review presentation을 사용한다 — 색상 count rail이 달린 rounded card, cluster label만 표시, 좌측 접기 rail 제공. cluster summary와 선택됨 badge는 숨긴다. Graph layout은 현재 edge weight 분포의 p05~p95 범위로 spring strength를 정규화하고, hard boundary clamp 대신 soft boundary force를 사용해 node가 viewport 가장자리에 일렬로 붙는 현상을 줄인다 `[현행 2026-07-13 → 15.169/15.238]`
