...
- **확정 시점**: 사용자가 `미션 종료` 버튼(구 `세션 종료`)을 누르면 `/api/memory/complete-session`에서 draft를 통합해 장기 메모리로 저장. 온보딩 제외 미션에서 제한 시간(`missionDurationMinutes`, 기본 30분) 전에 누르면 먼저 조기 종료 확인 문구가 뜨고, 확인 후 목업이 있는데 최종 디자인 미선택이면 기존 확인이 이어진다 `[현행 2026-07-21 → 15.325]`
